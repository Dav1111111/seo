...
For each probed query:
  ...
  - if competitor is in top-5 AND site is absent from the checked SERP
    sample → gap
...
SITE_SERP_DEPTH_FALLBACK = 10   # current Yandex Search API request defaults to top-10


@dataclasses.dataclass(frozen=True)
class GapRow:
    query: str
    site_position: int | None            # None = not visible in checked SERP sample
    serp_depth: int
...
def analyze_gaps(
    *,
    ...
    bigger gap = higher priority). Ties broken by query length asc.
    """
    ...
    for query, docs in query_to_serp.items():
        ...
        checked_depth = max(
            [
                int(_doc_get(d, "position") or 0)
                for d in docs
                if _doc_get(d, "position") is not None
            ]
            or [len(docs) or SITE_SERP_DEPTH_FALLBACK],
        )
